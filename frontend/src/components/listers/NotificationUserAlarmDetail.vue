<template>
    <v-card outlined>
        <v-card-title>
            UserAlarm # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <Number label="UserAlarmId" v-model="item.userAlarmId" :editMode="editMode" @change="change" />
            </div>
            <div>
                <Number label="UserId" v-model="item.userId" :editMode="editMode" @change="change" />
            </div>
            <div>
                <Number label="AlarmId" v-model="item.alarmId" :editMode="editMode" @change="change" />
            </div>
            <div>
                <Number label="EventId" v-model="item.eventId" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/userAlarms/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
